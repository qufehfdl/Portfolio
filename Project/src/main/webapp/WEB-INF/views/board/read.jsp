...
<body>
	...
	<div align="right">
		<a
			href="${root }board/main?board_category=${board_category}&page=${page}">📄
			목록보기</a>
		<c:if
			test="${loginUserBean.user_num == readContentBean.content_writer_num }">
			<a
				href="${root }board/modify?board_category=${board_category}&content_num=${content_num}&page=${page}">✏️
				수정하기</a>
			<a
				href="${root }board/delete?board_category=${board_category}&content_num=${content_num}">🌀
				삭제하기</a>
		</c:if>
	</div>
	<div>
	...
	<div>
		...
		<textarea id="board_content" name="board_content" rows="30"
			style="resize: none; width: 100%;" disabled="disabled">${readContentBean.content_text }</textarea>
	</div>
	<hr>
	<form:form action="${root }reply/write" modelAttribute="replyBean"
		method="post">
		<c:if test="${board_category == 5 || board_category == 6}">
			...
			<c:forEach items="${replyList}" var="list" varStatus="status">
				<c:if test="${list.getUser_num() == loginUserBean.user_num }">
						${list.getUser_name() }	🌠 
						<input type="text" id='modifyReply${status.index }'
						value="${list.getReply() }">
					<!-- onclick이벤트로 자바스크립트에 매개변수에다 여러 데이터값을 넘기는 방법 -->
					<button type="button"
						onclick="modifyReply('modifyReply${status.index }',${list.getReply_num() })">답글
						수정</button>
					<br>
				</c:if>
				<c:if test="${list.getUser_num() != loginUserBean.user_num }">
	...
	<c:if test="${readContentBean.content_file != null }">
		<div>
			<label for="board_file">첨부 이미지</label> <br> <img
				src="${root }upload/${readContentBean.content_file}" width="50%"
				height="200" />
		</div>
	</c:if>
</body>
